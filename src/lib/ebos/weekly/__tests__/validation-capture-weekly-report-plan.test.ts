...
describe("weekly plan validation capture integration", () => {
  it("prompts for external channel data when capture report has manual slots", () => {
    const report = createEmptyEbosReport("weekly", new Date("2026-07-03T12:00:00"));
    ...
    expect(plan.actionItems.some((item) => item.title.includes("外部渠道"))).toBe(true);
  });
});
