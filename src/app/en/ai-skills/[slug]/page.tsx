...
export default async function EnglishAiSkillDetailPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  return (
    <PublicSiteChrome forceLocale="en">
      <ToolDetailPageShell slug={slug} forceLocale="en" expectedType="ai_skill" />
    </PublicSiteChrome>
  );
}
