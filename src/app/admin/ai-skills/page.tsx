...
export default async function AdminAiSkillsPage({
  ...
  const where = buildAdminToolWhere("ai_skill", filters);
  ...
  return (
    <ToolAdminList
      title={locale === "en" ? "AI Skill Management" : "AI Skill 管理"}
      type="ai_skill"
      locale={locale}
      tools={tools}
      notice={params}
      ...
      total={total}
      pageCount={pageCount}
      buildPageHref={(page) => buildAdminToolPageHref("/admin/ai-skills", filters, page)}
    />
  );
}
