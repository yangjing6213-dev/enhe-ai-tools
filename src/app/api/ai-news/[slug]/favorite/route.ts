...
export async function POST(_request: Request, { params }: { params: Promise<{ slug: string }> }) {
  ...
  const existing = await prisma.newsArticleFavorite.findUnique({
    where: { articleId_userId: { articleId: article.id, userId: user.id } }
  });
  if (existing) return NextResponse.json({ ok: true, already: true });

  await prisma.$transaction([
    prisma.newsArticleFavorite.create({ data: { articleId: article.id, userId: user.id } }),
    prisma.newsArticle.update({ where: { id: article.id }, data: { favoriteCount: { increment: 1 } } })
  ]);
